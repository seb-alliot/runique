...
### Correctif — `derive_form` (parser `model!{}` : `meta: { abstract: true }` ne pouvait jamais s'analyser)

* **`abstract` est un mot-clé Rust réservé** — `Ident::parse` de `syn` le rejette purement et simplement, donc `meta: { abstract: true }` échouait systématiquement avec "expected identifier, found keyword `abstract`", peu importe une utilisation DSL par ailleurs correcte. Trouvé en ajoutant une couverture de tests unitaires dédiée au parsing de `meta:` (auparavant testé uniquement indirectement via `ModelInput`). La clé DSL est renommée en `is_abstract` — le champ sous-jacent n'est pas affecté. Si un modèle utilisait `meta: { abstract: true }`, le remplacer par `meta: { is_abstract: true }`.
