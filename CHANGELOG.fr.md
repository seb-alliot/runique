...
* **Trois nouveaux hooks `RuniqueForm`**, tous surchargeables par formulaire : `register_dynamic_fields(&mut self, request)` (champs dépendant de la requête, enregistrés avant que la validation ne tourne — ex. choix chargés depuis la base) ; `validator_get`/`validator_post(&self, request) -> bool` décident s'il faut tenter la validation pour les méthodes classe GET (GET/HEAD/OPTIONS/TRACE) et classe POST (POST/PUT/PATCH/DELETE/CONNECT) respectivement — les deux retombent par défaut sur `Forms::is_submitted()`, déjà `true` sans condition pour les méthodes classe POST et `true` seulement dès qu'un formulaire GET porte réellement des données (un formulaire de recherche en GET valide donc automatiquement dès qu'il est soumis, sans surcharge nécessaire).
* `ValidationForm::database_error(&mut self, err)` et `.into_inner()` laissent un handler soit continuer à travailler avec le wrapper (accès lecture seule via `Deref<Target = F>`, ex. pour enregistrer une erreur DB sans déballer), soit récupérer le `F` brut quand il a besoin de `&mut F`/`F` possédé pour sa propre logique de sauvegarde.
* Tous les handlers utilisant l'ancien pattern `is_get()`/`is_post()` migrés — `demo-app` (`info_user`, `handle_inscription`, `handle_login`, `handle_contribution_submit`, `handle_blog_save`, `handle_upload_image`), les handlers intégrés du framework (`forgot_password`/`password_reset`, `auth/password.rs`), et le scaffold que `runique new` génère (`composant-bin/code/views.rs`, vérifié en scaffoldant un vrai projet test et en le compilant contre la source locale). `ForgotPasswordForm`/`PasswordResetForm` surchargent explicitement `validator_get` à `false` — une action qui modifie l'état (émettre un token de reset, changer un mot de passe) ne doit jamais se déclencher sur une méthode "safe" quel que soit le contenu de la query string, ce que le dispatch par défaut basé sur `is_submitted()` seul ne garantirait pas.
* **`FormField::validate`/`finalize` sont désormais des `async fn`** (trait + les 19 implémentations par type de champ), et l'I/O d'upload de fichier de `file.rs` passe maintenant par du vrai `tokio::fs` + `spawn_blocking` au lieu du `block_in_place` précédent — un upload de fichier ou un hash de mot de passe dans un formulaire ne bloque plus le thread worker async sous la façade déjà async d'`is_valid()`. Un garde de récursion basé sur `tokio::task_local!` remplace un `thread_local!` devenu non fiable une fois la validation de champ capable de suspendre à travers un `.await` (une tâche reprenant sur un thread worker différent après suspension pouvait sinon corrompre le compteur de récursion d'une tâche sans rapport).
...
### Correctif — `runique` (générateur de migrations : ordre SQL invalide, casse d'identifiant Postgres, panique SQLite)

* **`ALTER COLUMN ... TYPE ... USING` combiné à une contrainte de nullabilité produisait du SQL invalide.** `modify_column(...)` chaînait `.not_null()`/`.null()` avec `.using(...)` pour les migrations de transition enum/type ; sea-query place `USING` à la toute fin de l'instruction `ALTER TABLE`, peu importe à quelle clause il « appartient », donnant `TYPE t, ALTER COLUMN x SET NOT NULL USING expr` — `USING` échoué après `SET NOT NULL` au lieu de juste après `TYPE`, rejeté par Postgres. Une transition enum/type ne change jamais la nullabilité, donc la contrainte n'est simplement plus posée sur ce `modify_column` précis.
* **Décalage de casse d'identifiant Postgres sur `ALTER TYPE ... ADD VALUE`/`RENAME VALUE`.** Le bloc `DO` du `CREATE TYPE` brut n'est jamais quoté, donc Postgres réduit le nom en minuscules à la création — mais le builder `Type::alter()` quote systématiquement son nom de type, casse préservée, donc il cherchait un identifiant différent, inexistant (`"ChangelogCategory"` au lieu du `changelogcategory` réellement stocké). Corrigé en passant le nom en minuscules uniquement à cet appel de builder précis ; `CREATE`/`DROP TYPE` et `ColumnType::Enum` n'ont pas eu besoin de changer, aucun des deux n'étant jamais quoté.
* **`modify_column` panique inconditionnellement sous SQLite.** Pas un rejet de syntaxe SQL — le backend SQLite de sea-query refuse de rendre *tout* `modify_column`, une vraie limite du moteur (le `ALTER TABLE` de SQLite ne supporte que `ADD`/`RENAME`/`DROP COLUMN`). Touchait toute migration de transition enum et tout changement général de nullable/type/unique dès qu'il atteignait ce moteur. Désormais sauté sous SQLite : sans perte pour les transitions enum (SQLite n'a pas d'enum natif — `ColumnType::Enum` s'y rend déjà en texte brut), et signalé par un commentaire d'avertissement explicite dans le fichier généré pour le cas général, où le changement de contrainte n'est, lui, réellement pas appliqué sur ce moteur.
* **Les clés étrangères sont désormais toujours déclarées inline dans le `CREATE TABLE`**, pas seulement pour SQLite. Un fichier de migration est figé pour toujours une fois committé, sa forme ne peut donc pas dépendre du moteur ciblé au moment de sa génération. Le mécanisme séparé de migration `..._create_relations` est supprimé — il aurait sinon déclaré la même contrainte en double.
* **Les triggers `updated_at` sont désormais toujours émis avec une vérification du backend au runtime**, pour la même raison : gardés auparavant au moment de la génération, un fichier généré en ciblant Postgres cassait silencieusement tous les autres moteurs.

### Correctif — `runique` (`makemigrations` : un `migration/` initialisé via `sea-orm-cli migrate init` produisait du Rust invalide)

* `sea-orm-cli migrate init` est l'étape documentée pour initialiser la crate `migration` (`Cargo.toml`, `src/main.rs`) avant de lancer `makemigrations` — mais son `lib.rs` généré a une forme différente de celle de Runique (un `vec![Box::new(X::Migration)]` sur une seule ligne, `pub use sea_orm_migration::prelude::*;`). L'ancien correctif ligne par ligne dans `update_migration_lib`/`ensure_admin_migration_positioned` supposait la mise en forme multi-lignes propre à Runique : les nouvelles entrées étaient insérées *après* le `vec![...]` déjà refermé au lieu de dedans, les migrations framework (`Eihwaz*`) atterrissaient hors de `Migrator::migrations()`, et la migration placeholder propre à `sea-orm-cli` (`up()`/`down()` tous deux un simple `todo!()`) n'était jamais retirée — elle aurait paniqué dès la première migration réellement exécutée. `lib.rs` est désormais analysé dans un petit modèle indépendant de la mise en forme (liste des modules + entrées du vec) puis toujours réécrit de façon canonique ; le module placeholder et son fichier sont supprimés dès le premier `makemigrations`.

### Correctif — `runique new` (scaffold : syntaxe `{% static %}` invalide faisait planter les nouveaux projets au démarrage)

* Le `templates/about/about.html` généré utilisait `{% static 'css/about.css' %}` (guillemets simples) — syntaxe invalide, faisant planter toute l'appli au chargement des templates avec `Unknown tag`. Corrigé en guillemets doubles, comme partout ailleurs.

### Correctif — `runique` (`CAST(... AS TEXT)` invalide sur MySQL/MariaDB — recherche et filtres)

* `CAST(col AS TEXT)` est une syntaxe Postgres/SQLite ; MySQL/MariaDB rejette `TEXT` comme type de cast et exige `CHAR`, car sea-query ne traduit jamais le nom du type cible selon le moteur. Touchait toute recherche insensible à la casse et tout filtre de colonne construit sur un `CAST` brut : les filtres de colonne `list_fn`/`count_fn` du générateur admin, la recherche des actions groupées, la requête d'options m2m, la résolution de label FK, les ressources intégrées `users`/`groupes`/`droits`, et les deux bras de la macro publique `search_cond!` (`all_columns icontains`, `or(...)`). Corrigé via de nouveaux helpers partagés `text_cast_type`/`text_eq`/`ilike` (`runique::admin::helper::sql_dialect`) qui choisissent `CHAR` ou `TEXT` selon `db.get_database_backend()` à l'exécution — un fichier de migration ou une ressource générée est figé une fois committé mais doit fonctionner correctement sur n'importe quel moteur utilisé par le projet, pas seulement celui configuré au moment de la génération.
* **Changement cassant pour les appelants de `search_cond!` :** chaque bras prend désormais la `&DatabaseConnection` vivante en premier argument — `search_cond!(db => Entity => or(...))` au lieu de `search_cond!(Entity => or(...))` — y compris les deux bras `?Col in`/`not_in` qui ne s'en servent pas en interne, car une macro ne peut pas mélanger des bras avec un nombre d'arguments différent sans ambiguïté de parsing entre un `$entity:ty` et un `$db:expr` en tête. `search!` n'est pas concernée (elle filtre via les méthodes typées `ColumnTrait`, déjà correctement rendues par sea-orm selon le moteur).

### Correctif — `runique` (la vérification CSRF ignorait le header `X-CSRF-Token` — cassait les clients JSON/fetch)

* La vérification CSRF de `Prisme` (`forms/extractor.rs`) ne lisait le token que dans le champ `csrf_token` du body/query parsé — Runique expose pourtant lui-même le token via un header de **réponse** `x-csrf-token` (pour qu'un client JS puisse le lire sans aller chercher un champ de formulaire caché), mais rien côté entrant ne le relisait jamais depuis un header de requête. Un client `fetch`/XHR envoyant le token en `X-CSRF-Token` avec un body JSON (sans champ `csrf_token`) échouait systématiquement la validation CSRF, donc `checked_data()` renvoyait `None` et tout code s'appuyant dessus voyait silencieusement une requête vide — pas d'erreur, juste des données jamais là. `check_csrf` retombe désormais sur le header `X-CSRF-Token` quand le body n'a pas de champ `csrf_token`. Sûr en tant que fallback : un `<form>` cross-site classique ne peut poser aucun header custom, et un `fetch`/XHR qui le fait déclenche un preflight CORS que le navigateur ne laisse passer que pour les origines explicitement autorisées par l'appli (`CorsConfig` désactivé par défaut, et la combinaison `any_origin() + allow_credentials(true)` est une erreur au build) — donc ce fallback n'ouvre aucune voie qu'une vérification body-only n'avait pas déjà à couvrir.
