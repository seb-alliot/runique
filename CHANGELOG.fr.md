...
* **Trois nouveaux hooks `RuniqueForm`**, tous surchargeables par formulaire : `register_dynamic_fields(&mut self, request)` (champs dépendant de la requête, enregistrés avant que la validation ne tourne — ex. choix chargés depuis la base) ; `validator_get`/`validator_post(&self, request) -> bool` décident s'il faut tenter la validation pour les méthodes classe GET (GET/HEAD/OPTIONS/TRACE) et classe POST (POST/PUT/PATCH/DELETE/CONNECT) respectivement. `validator_post` retombe par défaut sur `Forms::is_submitted()` (concrètement toujours `true`, ces méthodes n'étant jamais un premier chargement à vide) — les requêtes classe POST ne sont jamais exemptées de CSRF, auto-valider est donc sans risque. `validator_get` retombe par défaut sur **`false`**, sans condition : GET/HEAD sont les seules méthodes exemptées de CSRF, donc un formulaire qui auto-validerait sur GET par défaut laisserait ses effets de bord protégés par `is_valid()` (écriture en base, login, email) s'exécuter depuis un simple lien forgé, sans aucun token CSRF. Un formulaire de recherche/filtre en GET, en lecture seule, est le seul cas légitime qui doit auto-valider — il l'active explicitement (`fn validator_get(&self, _r: &Request) -> bool { self.get_form().is_submitted() }`), une ligne, plutôt que chaque formulaire qui modifie l'état doive penser à s'en désactiver.
