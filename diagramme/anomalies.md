...
| **F8** | ~~`ValidationForm::validator_get` par défaut sur `is_submitted()` → tout formulaire mutant (login/inscription/contact) auto-validait sur GET dès que la query string correspondait aux champs, exécutant son effet de bord **sans CSRF** (GET/HEAD seules méthodes exemptées)~~ | [forms/field.rs](../runique/src/forms/field.rs) | ✅ **CORRIGÉ (2026-09-22)** — défaut basculé à **`false`** inconditionnel ; seul un formulaire de recherche GET lecture-seule opte explicitement pour `is_submitted()` (`UsernameForm`, demo-app). Trouvé en migrant Campanile (aurait exigé 4 surcharges manuelles répétées côté app) |
...
| **E3** | Écart ordre d'écriture vs ordre d'exécution des `.layer()` Axum. **Conséquence concrète trouvée (2026-09-21) — 🟠 à reclasser sérieux** : `applicator.rs` slot 30 (`security_headers_middleware`, toujours actif, pose CSP+nonce) puis slot 31 (`csp_middleware`, ajouté dès `.with_csp()`, pose CSP **sans** nonce) — slot 31 étant appliqué après, il écrase le header du slot 30 dans `HeaderMap` (`insert`, pas `append`). Le nonce reste injecté côté requête (templates/`{% csp %}` fonctionnent) mais **disparaît du header final** → le navigateur bloque les scripts/styles inline nonce-tagués dès que `.with_csp()` est utilisé (usage documenté normal). `use_nonce`/`.with_nonce()` (champ+méthode jamais lus, supprimés le même jour) masquaient ce bug derrière une fausse impression de configurabilité. **Pas encore corrigé** — fix proposé : supprimer slot 31/`csp_middleware`, redondant (slot 30 fait déjà tout, en mieux) | [applicator.rs:303-328](../runique/src/app/staging/middleware_staging/applicator.rs#L303-L328), [csp.rs:211](../runique/src/middleware/security/csp.rs#L211) |
