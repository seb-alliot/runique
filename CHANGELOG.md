...
### Fix — `derive_form` (`model!{}` parser: `meta: { abstract: true }` could never parse)

* **`abstract` is a reserved Rust keyword** — `syn`'s `Ident::parse` rejects it outright, so `meta: { abstract: true }` failed with "expected identifier, found keyword `abstract`" on every attempt, regardless of correct DSL usage. Found while adding dedicated unit test coverage for `meta:` parsing (previously only exercised indirectly through `ModelInput`). The DSL key is renamed to `is_abstract` — the underlying field is unaffected. If any model used `meta: { abstract: true }`, replace it with `meta: { is_abstract: true }`.
