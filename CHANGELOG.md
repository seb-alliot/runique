...
* **`FormField::validate`/`finalize` are now `async fn`** (trait + all 19 field-type impls), and `file.rs`'s file-upload I/O now goes through real `tokio::fs` + `spawn_blocking` instead of the previous `block_in_place` shim — a form's file upload or password hashing no longer blocks the async worker thread under `is_valid()`'s already-async surface. A `tokio::task_local!`-based recursion guard replaces a prior `thread_local!` one that was unsound once field validation could suspend across an `.await` (a task resuming on a different worker thread after suspension could otherwise corrupt an unrelated task's recursion counter).
