...
* **Three new `RuniqueForm` hooks**, all overridable per form: `register_dynamic_fields(&mut self, request)` (fields depending on the request, registered before validation runs — e.g. choices loaded from the DB); `validator_get`/`validator_post(&self, request) -> bool` decide whether to attempt validation for GET-class (GET/HEAD/OPTIONS/TRACE) and POST-class (POST/PUT/PATCH/DELETE/CONNECT) requests respectively. `validator_post` defaults to `Forms::is_submitted()` (effectively always `true`, since these methods are never a bare first load) — POST-class requests are never CSRF-exempt, so auto-validating is safe. `validator_get` defaults to **`false`**, unconditionally: GET/HEAD are the only CSRF-exempt methods, so a form that auto-validated on GET by default would let its `is_valid()`-gated side effects (DB write, login, email) run from a plain crafted link, no CSRF token needed. A read-only GET search/filter form is the one legitimate case that should auto-validate — it opts in explicitly (`fn validator_get(&self, _r: &Request) -> bool { self.get_form().is_submitted() }`), one line, rather than every state-changing form having to remember to opt out.
