...
### Fix — `runique` (migration generator: invalid SQL ordering, Postgres identifier case-folding, SQLite panic)

* **`ALTER COLUMN ... TYPE ... USING` combined with a nullable spec produced invalid SQL.** `modify_column(...)` chained `.not_null()`/`.null()` together with `.using(...)` for enum/type-transition migrations; sea-query appends `USING` at the very end of the whole `ALTER TABLE` statement regardless of which clause it "belongs" to, so the result read `TYPE t, ALTER COLUMN x SET NOT NULL USING expr` — `USING` stranded after `SET NOT NULL` instead of right after `TYPE`, rejected by Postgres. An enum/type transition never changes nullability, so the nullable spec is simply no longer set on that particular `modify_column`.
* **Postgres identifier case-folding mismatch on `ALTER TYPE ... ADD VALUE`/`RENAME VALUE`.** The raw `CREATE TYPE` `DO` block is never quoted, so Postgres folds the name to lowercase at creation — but the `Type::alter()` builder always quotes its type name, preserving case, so it looked up a different, non-existent identifier (`"ChangelogCategory"` instead of the stored `changelogcategory`). Fixed by lowercasing the name passed to that specific builder call; `CREATE`/`DROP TYPE` and `ColumnType::Enum` needed no change since neither is ever quoted.
* **`modify_column` panics unconditionally on SQLite.** Not a SQL-syntax rejection — sea-query's SQLite backend refuses to render *any* `modify_column` at all, a real engine limitation (SQLite's `ALTER TABLE` only supports `ADD`/`RENAME`/`DROP COLUMN`). Affected every enum-transition and general nullable/type/unique-change migration once it reached that engine. Now skipped on SQLite: harmless for enum transitions (SQLite has no native enum — `ColumnType::Enum` already renders as plain text), and flagged with an explicit warning comment in the generated file for the general case, where the constraint change is genuinely not applied on that engine.
* **Foreign keys are now always declared inline in `CREATE TABLE`**, not only for SQLite. A migration file is fixed forever once committed, so its shape cannot depend on whichever engine `makemigrations` happened to target when it was generated. The separate `..._create_relations` migration mechanism is removed — it would otherwise double-declare the same constraint.
* **`updated_at` triggers are now always emitted with a runtime backend check**, for the same reason: previously gated at generation time, a file generated while targeting Postgres silently broke every other engine.

### Fix — `runique` (`makemigrations`: a `migration/` bootstrapped via `sea-orm-cli migrate init` produced invalid Rust)

* `sea-orm-cli migrate init` is the documented first step to scaffold the `migration` crate (`Cargo.toml`, `src/main.rs`) before ever running `makemigrations` — but its generated `lib.rs` uses a different shape than Runique's own (a single-line `vec![Box::new(X::Migration)]`, `pub use sea_orm_migration::prelude::*;`). The old line-based patching in `update_migration_lib`/`ensure_admin_migration_positioned` assumed Runique's own multi-line layout: new entries were inserted *after* the whole, already-closed `vec![...]` statement instead of inside it, the framework migrations (`Eihwaz*`) landed outside `Migrator::migrations()` entirely, and `sea-orm-cli`'s own placeholder migration (`up()`/`down()` both a bare `todo!()`) was never removed — it would panic the first time any migration actually ran. `lib.rs` is now parsed into a small, formatting-independent model (module list + vec entries) and always re-rendered canonically; the placeholder module and its file are dropped on the very first `makemigrations` run.

### Fix — `runique new` (scaffold: invalid `{% static %}` syntax crashed new projects at boot)

* The generated `templates/about/about.html` used `{% static 'css/about.css' %}` (single quotes) — invalid syntax, failing the whole app at template-load time with `Unknown tag`. Fixed to double quotes, matching every other template.

### Fix — `runique` (`CAST(... AS TEXT)` invalid on MySQL/MariaDB — search and filters)

* `CAST(col AS TEXT)` is Postgres/SQLite syntax; MySQL/MariaDB reject `TEXT` as a cast target and require `CHAR`, since sea-query never translates the cast type name across backends. Affected every case-insensitive search and column filter built on a raw `CAST`: the admin generator's `list_fn`/`count_fn` column filters, the bulk-action lookup, the m2m option query, the FK label lookup, the built-in `users`/`groupes`/`droits` resources, and both arms of the public `search_cond!` macro (`all_columns icontains`, `or(...)`). Fixed with new shared helpers `text_cast_type`/`text_eq`/`ilike` (`runique::admin::helper::sql_dialect`) that pick `CHAR` vs `TEXT` from `db.get_database_backend()` at runtime — a migration file or generated resource is committed once but must run correctly on whichever engine the project actually uses, not whichever engine happened to be configured when it was generated.
* **Breaking for `search_cond!` callers:** every arm now takes the live `&DatabaseConnection` as its first argument — `search_cond!(db => Entity => or(...))` instead of `search_cond!(Entity => or(...))` — including the two `?Col in`/`not_in` arms that don't need it internally, since a macro can't mix arms with different argument counts without introducing parsing ambiguity between a leading `$entity:ty` and a leading `$db:expr`. `search!` is unaffected (it filters through typed `ColumnTrait` methods, which sea-orm already renders correctly per backend).

### Fix — `runique` (CSRF check ignored the `X-CSRF-Token` header — broke JSON/fetch clients)

* `Prisme`'s CSRF check (`forms/extractor.rs`) only ever read the `csrf_token` field from the parsed body/query — Runique itself exposes the token via an `x-csrf-token` **response** header (so JS clients can read it without scraping a hidden form field), but nothing on the way back **in** ever read it from a request header. A `fetch`/XHR client sending the token as `X-CSRF-Token` with a JSON body (no `csrf_token` field) always failed CSRF validation, so `checked_data()` returned `None` and any code relying on it silently saw an empty request — no error, just data that was never there. `check_csrf` now falls back to the `X-CSRF-Token` header when the body has no `csrf_token` field. Safe as a fallback: a plain cross-site `<form>` can't set custom headers at all, and a `fetch`/XHR that does triggers a CORS preflight the browser only completes for origins the app explicitly allowed (`CorsConfig` is disabled by default, and its `any_origin() + allow_credentials(true)` combination is a build-time error) — so this opens no path a body-only check didn't already have to guard against.
...
### Fix — `runique` (session DB backup: authenticated sessions lost on restart)
...
### Fix — `runique` (error pages: responsive + i18n)
