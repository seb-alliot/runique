...
- ✅ **`ValidationForm<F>` — validation typestate** — `try_new(form, request) -> Result<ValidationForm<F>, F>` : `Ok` uniquement après validation réussie, preuve par le type qu'un handler a bien validé avant d'agir. Dispatch par défaut sur `Method::is_safe()` (`validator_get`/`validator_post`, tous deux réutilisant `is_submitted()` par défaut), hooks `register_dynamic_fields`/`validator_get`/`validator_post` surchargeables par formulaire. Inspiré de `FormMixin`/`form_valid()` Django, adapté aux garanties de typage statique Rust — additif, non-breaking. **Tout l'ancien pattern `is_get()`/`is_post()` éliminé** : tous les handlers `demo-app` migrés (`info_user`, `handle_inscription`, `handle_login`, `handle_contribution_submit`, `handle_blog_save`, `handle_upload_image`), les handlers intégrés du framework (`auth/password.rs::handle_forgot_password`/`handle_password_reset`, avec `validator_get: false` explicite — une action qui modifie l'état ne doit jamais se déclencher sur une méthode "safe") et le scaffold `composant-bin/code/views.rs` généré par `runique new` (vérifié en scaffoldant un vrai projet test et en le compilant contre la source locale)
