...
/// Générer la validation pour RustiForm (nouveau)
pub(crate) fn generate_validation_rustiform(field: &Field) -> proc_macro2::TokenStream {
...
/// Déterminer le type de champ Rusti à partir du type Rust (pour register_field)
pub(crate) fn get_field_type(field: &Field) -> proc_macro2::TokenStream {
    ...
    if field_name.contains("url") || field_name.contains("link") || field_name.contains("website") {
        return quote! { ::rusti::formulaire::field::URLField };
    }
