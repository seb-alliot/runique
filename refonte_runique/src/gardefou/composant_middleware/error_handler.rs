use crate::gardefou::composant_middleware::error_handler;
...
fn render_debug_error(
    ...
    template_context.insert("static_runique", &config.static_runique_url);
...
fn critical_error_html(error: &str, tera: &Tera, context: &Context, config: &RuniqueConfig) -> Response {
    let mut context = context.clone();
    context.insert("static_runique", &config.static_runique_url);
