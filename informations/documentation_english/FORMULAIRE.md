# Forms Guide - Rusti Framework

Rusti provides a Django-inspired form system with automatic generation via procedural macros.
...
Rusti provides **two macros** to automatically generate forms from your SeaORM models:

1. **`#[rusti_form]`** - For creating custom forms
2. **`#[derive(DeriveModelForm)]`** - For generating model-linked forms

### `#[rusti_form]` Macro

This macro automatically generates the implementation of the `RustiForm` trait.
...
##### b) Implementation of `RustiForm` Trait
...
| Element | Generated by `#[rusti_form]` | Generated by `#[derive(DeriveModelForm)]` |
|---------|------------------------------|------------------------------------------|
...
CSRF protection is **automatically activated** in Rusti when the `CsrfMiddleware` is added.
...
Create robust forms with Rusti!
