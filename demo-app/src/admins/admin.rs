...
pub fn admin_register() -> AdminRegistry {
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "user_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "contribution_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "content", escaped)));
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "email", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "website", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "summary", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "content", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "version", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "release_date", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "category", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "description", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let rows: Vec<serde_json::Value> = db_rows.into_iter()
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "status", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "description", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "link_url", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "link_label", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "link_url_2", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "link_label_2", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "description", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "issue_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "category_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "slug", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "lead", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "content", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "language", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "code", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "context", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "label", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "url", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "link_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "name", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "field_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "description", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "example", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "html_preview", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "slug", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "lang", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "theme", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "section_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "slug", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "lang", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "lead", escaped)));
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "page_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "content", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "block_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "heading", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "theme", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "difficulte", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "ordre", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "cour_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "slug", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "title", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                let escaped = search_str.replace('\'', "''");
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "chapitre_id", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "block_type", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "heading", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "sort_order", escaped)));
                query = query.filter(search_cond);
    ...
    let list_fn: ListFn = Arc::new(|db: ADb, params: ListParams| {
        Box::pin(async move {
            ...
            if let Some(ref search_str) = params.search {
                ...
                let mut search_cond = sea_orm::Condition::any();
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "version", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "github_url", escaped)));
                search_cond = search_cond.add(Expr::cust(format!("LOWER(CAST({} AS TEXT)) LIKE LOWER('%%{}%%')", "crates_url", escaped)));
                query = query.filter(search_cond);
            }
            let db_rows = query.offset(params.offset).limit(params.limit).all(&*db).await?;
            let rows: Vec<serde_json::Value> = db_rows.into_iter()
    ...
    registry.configure_group_actions("users", vec![GroupAction::bool("is_active", "Actif"), GroupAction::bool("is_staff", "Staff")]);
    registry
}
