...
pub fn admin_register() -> AdminRegistry {
    ...
    let form_builder: FormBuilder = Arc::new(|data: StrMap, tera: ATera, csrf: String, method: Method| {
        Box::pin(async move {
            let form = eihwaz_users::AdminForm::build_with_data(&data, tera, &csrf, method).await;
    ...
    let count_fn: CountFn = Arc::new(|db: ADb, _search: Option<String>| {
        Box::pin(async move {
            eihwaz_users::Entity::find().count(&*db).await
        })
    });
    ...
    let delete_fn: DeleteFn = Arc::new(|db: ADb, id: String| {
        Box::pin(async move {
            let id = id.parse::<i32>().map_err(|_| DbErr::Custom("id invalide".to_string().to_string()))?;
            eihwaz_users::Entity::delete_by_id(id).exec(&*db).await.map(|_| ())
        })
    });
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_stmt_username = Query::select().expr(Expr::cust("COUNT(DISTINCT username)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("username")).is_not_null()).to_owned();
            ...
            let stmt_username = Query::select().distinct().expr(Expr::cust("CAST(username AS TEXT)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("username")).is_not_null()).limit(page_size_username).offset(cur_page_username * page_size_username).to_owned();
            ...
            let count_stmt_email = Query::select().expr(Expr::cust("COUNT(DISTINCT email)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("email")).is_not_null()).to_owned();
            ...
            let stmt_email = Query::select().distinct().expr(Expr::cust("CAST(email AS TEXT)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("email")).is_not_null()).limit(page_size_email).offset(cur_page_email * page_size_email).to_owned();
            ...
            let count_stmt_is_superuser = Query::select().expr(Expr::cust("COUNT(DISTINCT is_superuser)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("is_superuser")).is_not_null()).to_owned();
            ...
            let stmt_is_superuser = Query::select().distinct().expr(Expr::cust("CAST(is_superuser AS TEXT)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("is_superuser")).is_not_null()).limit(page_size_is_superuser).offset(cur_page_is_superuser * page_size_is_superuser).to_owned();
            ...
            let count_stmt_is_active = Query::select().expr(Expr::cust("COUNT(DISTINCT is_active)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("is_active")).is_not_null()).to_owned();
            ...
            let stmt_is_active = Query::select().distinct().expr(Expr::cust("CAST(is_active AS TEXT)")).from(Alias::new(eihwaz_users::Entity.table_name())).and_where(Expr::col(Alias::new("is_active")).is_not_null()).limit(page_size_is_active).offset(cur_page_is_active * page_size_is_active).to_owned();
