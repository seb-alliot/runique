...
pub fn admin_register() -> AdminRegistry {
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_title = match db.query_one(&count_stmt_title).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `cour.title` : colonne introuvable en DB — {}", e); None } };
            let total_title = count_row_title.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            ...
            let rows_title = match db.query_all(&stmt_title).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `cour.title` : colonne introuvable en DB — {}", e); vec![] } };
            let mut vals_title: Vec<String> = rows_title.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_title.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            result.insert("title".to_string(), (vals_title, total_title));
            let page_size_theme = 10u64;
            let cur_page_theme = pages.get("theme").copied().unwrap_or(0);
    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_title = match db.query_one(&count_stmt_title).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `chapitre.title` : colonne introuvable en DB — {}", e); None } };
            let total_title = count_row_title.and_then(|r| r.try_get_by_index::<i64>(0).ok()).unwrap_or(0) as u64;
            ...
            let rows_title = match db.query_all(&stmt_title).await { Ok(r) => r, Err(e) => { tracing::error!("[runique admin] list_filter `chapitre.title` : colonne introuvable en DB — {}", e); vec![] } };
            let mut vals_title: Vec<String> = rows_title.iter().filter_map(|r| r.try_get_by_index::<String>(0).ok()).collect(); vals_title.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) { (Ok(x), Ok(y)) => x.cmp(&y), _ => a.cmp(b) });
            result.insert("title".to_string(), (vals_title, total_title));
            let page_size_sort_order = 10u64;
            let cur_page_sort_order = pages.get("sort_order").copied().unwrap_or(0);
