    ...
    let filter_fn: FilterFn = Arc::new(|db: ADb, pages: std::collections::HashMap<String, u64>| {
        Box::pin(async move {
            ...
            let count_row_username = match db.query_one(&count_stmt_username).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.username` : colonne introuvable en DB — {}",
                        e
                    );
                    None
                }
            ...
            let count_row_email = match db.query_one(&count_stmt_email).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.email` : colonne introuvable en DB — {}",
                        e
                    );
                    None
                }
            ...
            let count_row_is_superuser = match db.query_one(&count_stmt_is_superuser).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.is_superuser` : colonne introuvable en DB — {}",
                        e
                    );
                    None
                }
            ...
            let stmt_is_superuser = Query::select()
                ...
                .to_owned();
            let rows_is_superuser = match db.query_all(&stmt_is_superuser).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.is_superuser` : colonne introuvable en DB — {}",
            ...
            let mut vals_is_superuser: Vec<String> = rows_is_superuser
                .iter()
                .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                .collect();
            vals_is_superuser.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) {
                (Ok(x), Ok(y)) => x.cmp(&y),
                _ => a.cmp(b),
            });
            result.insert(
            ...
            let count_row_is_active = match db.query_one(&count_stmt_is_active).await {
                ...
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.is_active` : colonne introuvable en DB — {}",
                        e
                    );
                    None
                }
            ...
            let stmt_is_active = Query::select()
                ...
                .to_owned();
            let rows_is_active = match db.query_all(&stmt_is_active).await {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!(
                        "[runique admin] list_filter `users.is_active` : colonne introuvable en DB — {}",
            ...
            let mut vals_is_active: Vec<String> = rows_is_active
                .iter()
                .filter_map(|r| r.try_get_by_index::<String>(0).ok())
                .collect();
            vals_is_active.sort_by(|a, b| match (a.parse::<i64>(), b.parse::<i64>()) {
                (Ok(x), Ok(y)) => x.cmp(&y),
                _ => a.cmp(b),
            });
