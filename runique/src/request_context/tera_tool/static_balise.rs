use crate::request_context::tera_tool::form;
use crate::request_context::tera_tool::url_balise;
use std::collections::HashMap;
use std::sync::{Arc, RwLock};
use tera::{Result as TeraResult, Tera, Value};

// Filtre pour générer un champ CSRF hidden (équivalent Django {% csrf_token %})
fn csrf_field_filter(value: &Value, _: &HashMap<String, Value>) -> TeraResult<Value> {
    let token = value
        .as_str()
