...
fn generate_router_file(resources: &[ResourceDef], dir: &Path) -> Result<(), String> {
    ...
    for r in resources {
        ...
        let _ = writeln!(
            out,
            "        &format!(\"{{}}/{}/list\", prefix) => view! {{ handlers::{}_list }}, name = \"{}_list\",",
            key, key, key
        );
        let _ = writeln!(
            out,
            "        &format!(\"{{}}/{}/create\", prefix) => view! {{ handlers::{}_create }}, name = \"{}_create\",",
            key, key, key
        );
        let _ = writeln!(
            out,
            "        &format!(\"{{}}/{}/{{{{id}}}}\", prefix) => view! {{ handlers::{}_detail }}, name = \"{}_detail\",",
            key, key, key
        );
        let _ = writeln!(
            out,
            "        &format!(\"{{}}/{}/{{{{id}}}}/edit\", prefix) => view! {{ handlers::{}_edit }}, name = \"{}_edit\",",
            key, key, key
        );
        let _ = writeln!(
            out,
            "        &format!(\"{{}}/{}/{{{{id}}}}/delete\", prefix) => view! {{ handlers::{}_delete }}, name = \"{}_delete\",",
            key, key, key
        );
    }
    let _ = writeln!(out, "    }}");
...
fn write_handler_list(out: &mut String, r: &ResourceDef) -> Result<(), String> {
    ...
    let _ = writeln!(out);
    let _ = writeln!(out, "        let resource = admin");
    ...
    let _ = writeln!(out, "        }});");
    let _ = writeln!(out, "        return req.render(\"list\");");
    let _ = writeln!(out, "    }}");
    let _ = writeln!(out);
    ...
    let _ = writeln!(
        out,
        "            return Ok(Redirect::to(&format!(\"{{}}/{}/list\", admin.config.prefix.trim_end_matches('/'))).into_response());",
        key
    );
    ...
    let _ = writeln!(out, "    }}");
    let _ = writeln!(out);
    let _ = writeln!(out, "    req.render(\"list\")");
    let _ = writeln!(out, "}}");
    let _ = writeln!(out);
    Ok(())
}
...
fn write_handler_create(out: &mut String, r: &ResourceDef) -> Result<(), String> {
    ...
    let _ = writeln!(out, "        }});");
    let _ = writeln!(out, "        return req.render(\"form\");");
    let _ = writeln!(out, "    }}");
    let _ = writeln!(out);
    let _ = writeln!(out, "    if req.is_post() {{");
    let _ = writeln!(out, "        if form.is_valid().await {{");
    ...
    let _ = writeln!(out, "            context_update!(req => {{");
    let _ = writeln!(out, "                \"resource_key\" => \"{}\",", key);
    let _ = writeln!(out, "                \"current_resource\" => \"{}\",", key);
    let _ = writeln!(out, "                \"resource\" => resource,");
    let _ = writeln!(out, "                \"form_fields\" => &form,");
    let _ = writeln!(out, "                \"is_edit\" => false,");
    let _ = writeln!(
    ...
    let _ = writeln!(out, "    }}");
    let _ = writeln!(out);
    let _ = writeln!(out, "    req.render(\"form\")");
    let _ = writeln!(out, "}}");
    let _ = writeln!(out);
    Ok(())
}
...
fn write_handler_detail(out: &mut String, r: &ResourceDef) -> Result<(), String> {
    ...
    let _ = writeln!(out, "        \"is_edit\" => false");
    let _ = writeln!(out, "    }});");
    let _ = writeln!(out, "    req.render(\"detail\")");
    let _ = writeln!(out, "}}");
    let _ = writeln!(out);
    Ok(())
}
...
fn write_handler_delete(out: &mut String, r: &ResourceDef) -> Result<(), String> {
    ...
    let _ = writeln!(out, "        \"is_edit\" => false");
    let _ = writeln!(out, "    }});");
    let _ = writeln!(out, "    req.render(\"delete\")");
    let _ = writeln!(out, "}}");
    let _ = writeln!(out);
    Ok(())
}
