use runique::admin::{
    AdminConfig, AdminRegistry, AdminResource, DisplayConfig, ResourcePermissions,
};
...
use runique::migration::{
    ColumnDef, ForeignKeyDef, HooksDef, IndexDef, ModelSchema, PrimaryKeyDef, RelationDef,
};
...
fn smoke_form_fields_validation() {
    let mut text = TextField::text("username")
        .min_length(3, "")
        .max_length(10, "");
    text.set_value("abcd");
...
fn smoke_admin_registry_and_routes_helpers() {
    ...
    let res =
        AdminResource::with_permissions("users", "users::Model", "RegisterForm", "Users", perms)
