...
pub async fn connect() -> Option<DatabaseConnection> {
    ...
    match Database::connect(&url).await {
        Ok(db) => Some(db),
        Err(e) => panic!(
            "DATABASE_URL_PG is set ({url}) but the Postgres container is unreachable: {e}\n\
             Run `docker compose up -d`, or unset DATABASE_URL_PG in .env.test to skip Postgres tests."
        ),
    }
}
