...
It is currently not possible to customize a single field (e.g. add `.max_size(5)` or change the label) without rewriting the entire `register_fields` by hand, which defeats the purpose of the macro.
