...
It is currently not possible to customize a single field (e.g. add `.max_size(FileSize::mb(5))` or change the label) without rewriting the entire `register_fields` by hand, which defeats the purpose of the macro.
...
This limitation will be addressed in a future version with the introduction of per-field overrides (`overrides = [...]`).

> **Important:** in a manual form, the `max_size` set on the `FileField` and the one declared in `model!{}` are two independent values — they are not automatically synchronized. They must be aligned by hand. For forms generated via `#[form(...)]`, the model's limit is applied directly in `register_fields`.
