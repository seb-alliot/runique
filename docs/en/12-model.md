...
The DSL is converted into a `Model` AST containing, among other things:
...
After AST parsing, the generation phase builds, among other things:
...
* If `fields` is provided: it acts as a prioritized whitelist (order preserved).
...
* an `impl ModelForm` (`schema()`, `fields()`, `exclude()`),
* an `impl RuniqueForm` delegating to `ModelForm::model_register_fields(...)`.
...
* Single centralized model/schema contract
* Consistent migration + form generation
* Reduced duplication of field definitions

### Points of Attention

* Strict DSL: syntax errors result in macro errors at build time
* Misalignment between `fields`/`exclude` and the schema leads to generation/runtime errors
* Pedagogical order matters: understand `model/schema` before the model-based form method
