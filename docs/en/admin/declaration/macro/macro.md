...
**Unique-constrained fields are automatically excluded** from the bulk edit form — setting the same unique value on multiple records would violate the constraint. These fields are detected via the `UNIQUE_FIELDS` constant generated by `model!{}` for each entity.
