...
The language set here applies to user-facing messages: form validation and error messages. Internal logs (`tracing::warn!`/`error!`) are, for the vast majority, hardcoded in **English** in the source — some even go through `t()`/`tf()` and are therefore translated. They don't consistently follow the configured language.
