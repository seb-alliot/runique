...
`LoginGuard` s'utilise **dans le handler**, pas comme middleware — c'est la seule façon d'accéder au username soumis dans le formulaire sans consommer le body avant l'extraction via `request.form()`.
