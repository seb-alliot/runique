...
> En syntaxe v2, le bloc `form_fields:` est ignoré — les types sémantiques contiennent déjà les informations de widget.
...
model! {
    ...
    fields: {
        statut:   enum(StatutCommande) [required],
        priorite: enum(Priorite)       [required],
    },
}
```

### Trois formes de variant
...
| `file(image)`                 | `{MEDIA_ROOT}/{nom_du_champ}/`          |
...
relations: {
    ...
    belongs_to: Model via champ_fk [cascade],
    has_many: Model,
    has_one: Model,
...
|------------------|-----------------|------------------------------|
| `belongs_to`     | ✅ `FOREIGN KEY` | Clé étrangère vers `model.id` |
| `has_many`       | ❌ code seul     | Relation 1-N                 |
| `has_one`        | ❌ code seul     | Relation 1-1                 |
| `many_to_many`   | ❌ code seul     | Relation N-N via pivot       |

Actions FK disponibles : `cascade` · `restrict` · `set_null` · `set_default`
...
```rust
fields: {
    titre:        String [required, label("Titre de l'article")],
