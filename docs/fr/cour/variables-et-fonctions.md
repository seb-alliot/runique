...
**Variables, mutabilité et fonctions** — guide complet pour débutants.
...
En Rust, les variables sont déclarées avec le mot-clé `let`. Une des particularités de Rust est
que **les variables sont immutables par défaut**.
...
> **Note :** en Rust, on utilise `println!` (avec un `!`) pour afficher du texte. Le `{}` est
> remplacé par la valeur de la variable.

### 1.2 — Immutabilité par défaut

Par défaut, les variables Rust sont **immutables** : impossible de changer leur valeur après
leur déclaration.
...
> **Pourquoi ?** L'immutabilité par défaut aide à éviter les bugs. Pour modifier une variable,
> il faut le déclarer explicitement avec `mut`.
...
> **Conseil :** utiliser `mut` seulement quand nécessaire — rend le code plus sûr et plus facile
> à comprendre.

### 1.4 — Les constantes

Les constantes sont déclarées avec `const` et sont **toujours immutables**. Elles doivent avoir
un type explicite et sont évaluées à la compilation.
...
| | `let` | `const` |
|---|---|---|
| Immutable par défaut | ✅ (sauf si `mut`) | ✅ (toujours) |
| Peut être `mut` | ✅ Oui | ❌ Jamais |
| Type explicite requis | ❌ Non (inférence possible) | ✅ Oui, toujours |
| Portée | Bloc | Globale |
| Valeur calculée | À l'exécution | À la compilation |
| Convention de nom | `snake_case` | `UPPER_CASE` |

### 1.5 — Le shadowing

Le **shadowing** permet de redéclarer une variable avec le même nom — différent de la
mutabilité.
...
let x = x + 1;  // Nouvelle variable qui masque la précédente
println!("x = {}", x);  // 6
...
let espaces = espaces.len();   // Type : usize
println!("Il y a {} espaces", espaces);  // 3
```

**Shadowing vs `mut` :**

- `mut` : modifie la valeur, **même type obligatoire**.
- `let` (shadowing) : crée une nouvelle variable, **peut changer de type**.

## 2. Les types de données

Rust est un langage **statiquement typé** : chaque variable a un type connu à la compilation.
Rust peut souvent **inférer** le type, mais on peut aussi l'annoter explicitement.
...
Les fonctions sont omniprésentes en Rust. Elles sont déclarées avec `fn` et utilisent la
...
fn ma_fonction() { }        // snake_case
fn MaFonction() { }         // à éviter
fn calculer_total() { }     // snake_case, correct
fn calculerTotal() { }      // à éviter (camelCase)
```
...
Une fonction retourne une valeur avec `->` suivi du type. La dernière expression est retournée
automatiquement (pas de `return` nécessaire).
...
> **Attention !** En Rust, une expression **sans point-virgule** est retournée. Avec
> point-virgule, c'est une instruction qui ne retourne rien.
...
```rust
// Instruction (ne retourne rien)
fn plus_un_incorrect(x: i32) -> i32 {
    x + 1;  // Avec ; = ne retourne rien !
...
L'**ownership** est le concept le plus important et unique de Rust. C'est ce qui permet à Rust
d'être sûr sans ramasse-miettes (*garbage collector*).
...
fn main() {
    ...
    // println!("{}", texte);  // ERREUR ! texte a été déplacé dans la fonction
}
```

> **Types `Copy` :** les types simples (`i32`, `f64`, `bool`, `char`) sont `Copy` — ils sont
> copiés au lieu d'être déplacés.

### 4.2 — Les références (`&`)

Les **références** permettent d'emprunter une valeur sans en prendre l'ownership.
...
fn main() {
    ...
    println!("'{}' a {} caractères", texte, longueur); // texte est toujours valide !
}
```

```rust
// Multiples références immutables OK
fn main() {
...
fn main() {
    ...
    let r1 = &mut s;
    // let r2 = &mut s;  // ERREUR ! On ne peut pas avoir deux références mutables
    println!("{}", r1);
}
```
...
> **Règles du borrowing :**
> 1. Soit **une** référence mutable.
> 2. Soit **plusieurs** références immutables.
> 3. Mais **jamais les deux en même temps**.

## 5. Exercices pratiques

### Exercice 1 — Variables

Compléter ce code pour qu'il compile :
...
Créer une fonction qui multiplie deux nombres :
...
Créer une fonction qui double un nombre sans le déplacer :
...
Ce cours couvre les bases de Rust — variables, types, fonctions, ownership et borrowing.

**Prochaines étapes :**
- Pratiquer avec de petits programmes.
- Apprendre les structures (`struct`) et les enums — voir `structures-enums.md`.
- Maîtriser le pattern matching — voir `structures-et-controle.md`.
- Explorer les collections (`Vec`, `HashMap`) — voir `collections.md`.

> Continue à coder et n'aie pas peur des erreurs du compilateur : elles sont là pour aider.
