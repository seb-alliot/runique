...
| `Lang::Es` | `es` | `es`, `es-ES`, `es-MX`, `es-AR`, `es-CO`, `es-CL` |
...
La langue définie ici s'applique aux messages visibles par l'utilisateur : validation de formulaires et messages d'erreur. Les logs internes (`tracing::warn!`/`error!`) sont très majoritairement codés en dur **en anglais** dans le source — certains passent même par `t()`/`tf()` et sont donc traduits. Ils ne suivent pas systématiquement la langue configurée.
