...
La langue définie ici s'applique à tous les messages du framework : validation de formulaires, messages d'erreur, et logs internes (`tracing::warn!`). Les avertissements émis par le framework (ex : échec de parsing) seront donc dans la langue configurée.
