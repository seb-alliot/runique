...
> Le filtre `markdown` déclare `is_safe()` — son HTML est émis sans échappement, `\| safe` est inutile et n'a pas besoin d'être ajouté manuellement.
>
...
> `sanitize` relance **ammonia au moment du rendu** ; comme `markdown`, il déclare `is_safe()` directement (pas de `\| safe` à ajouter), donc le HTML émis est toujours fraîchement nettoyé — la sanitisation se fait à la **sortie**, sans jamais faire confiance à ce qui est stocké. À utiliser pour afficher un champ rich-text en HTML rendu.
>
...
| Filtre | Description | Exemple |
|--------|-------------|---------|
| `form` | Rendu complet du formulaire | `{{ nom_form \| form }}` |
| `form(field='xxx')` | Rendu d'un seul champ | `{{ nom_form \| form(field='email') }}` |
| `csrf_field` | Génère un input hidden CSRF | `{{ csrf_token \| csrf_field }}` |

Les trois déclarent `is_safe()` — leur sortie est émise sans échappement, `\| safe` est inutile.
